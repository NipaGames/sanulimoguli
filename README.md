...
$$
E[I]=\sum_{x\in I}^{ }p\left(x\right)\cdot\log_2\left(\frac{1}{p\left(x\right)}\right)\\\textup{jossa \textit{p(x)} tarkoittaa \textit{x}:n todennäköisyyttä ja \textit{I} sisältää kaikki \textit{x}:t eli vihjeiden määrät}
$$
<br>
...
$$
log_2\left(\frac{1}{p\left(x\right)}\right)=log_2\left(\frac{1}{0,25}\right)=log_2(4)=2
$$
